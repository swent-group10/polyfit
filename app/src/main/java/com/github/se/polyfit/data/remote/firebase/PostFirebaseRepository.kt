    ...
    return documentTask.continueWithTask { task ->
      if (task.isSuccessful) {
        ...
        if (post.listOfImages.isNotEmpty()) {
          val uploadTask = post.listOfImages.map { uploadImage(it, documentRef) }
        ...
        Log.e("PostFirebaseRepository", "Failed to store post in the database", task.exception)
  ...
  private fun uploadImage(image: Bitmap, documentRef: DocumentReference): Task<Void> {
    ...
    return refSource.putStream(stream).continueWithTask { uploadTask ->
      if (uploadTask.isSuccessful) {
        ...
        Log.e("PostFirebaseRepository", "Failed to upload image", uploadTask.exception)
