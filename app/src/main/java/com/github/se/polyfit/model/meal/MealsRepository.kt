...
class MealsRepository(private val db: FirebaseFirestore = FirebaseFirestore.getInstance()) {
    ...
    fun addMeal(uid: String, meal: Meal): Task<Void> {
        ...
        val addMealTask = newMealDoc.set(serializedData.first)
            ...
            .addOnSuccessListener {
                Log.e(
                    "FirebaseConnection",
                    "DocumentSnapshot added with ID: ${newMealDoc.id}"
                )
            }

        serializedData.second.forEach { ingredient ->
            addMealTask.continueWithTask {
                newMealDoc.collection("ingredients").add(ingredient)
            }.addOnFailureListener({ e ->
                Log.e("FirebaseConnection", "Error adding ingredient", e)
            })
        }
    ...
    fun getAllMeals(uid: String): Task<List<Meal>> {
        ...
        userCollection.document(uid).collection("meals").get()
            .addOnSuccessListener { documents ->
                val mealTasks = ArrayList<Task<Meal>>()

                documents.forEach { document ->
                    val mealTask = document.reference.collection("ingredients").get()
                        .continueWithTask { ingredientTask ->
                            val ingredients = ingredientTask.result?.mapNotNull { ingredientDoc ->
                                ingredientDoc.data
                            }.orEmpty()
                            ...
                            if (data != null) {
                                Tasks.forResult(
                                    Meal.deserializeMeal(data, ingredients)
                ...
                Tasks.whenAllSuccess<Meal>(mealTasks).addOnSuccessListener { meals ->
                    taskSource.setResult(meals)
    ...
    fun updateMeal(
        ...
        userCollection.document(uid).collection(("meals")).document(meal.getUid())
